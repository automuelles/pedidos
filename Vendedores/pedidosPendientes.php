...
try {
    // Consulta para obtener las facturas en estado "RevisionFinal" junto con novedades, ordenadas por fecha descendente
    $sql = "SELECT f.*, n.novedad, n.descripcion 
            FROM factura f
            LEFT JOIN Novedades_Bodega n ON f.id = n.factura_id
            WHERE f.estado = 'RevisionFinal'
            ORDER BY f.fecha DESC"; // Ordenar por fecha, de más reciente a más antigua
